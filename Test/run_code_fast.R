...
#--------------set_up---------------------------
n_simulation_MSE_vs_PCA <- c(5)
T_simulation_MSE_vs_PCA <- c(7)
number_samples_MSE_vs_PCA <- 2 #must be at least 2
...
#--------------set_up---------------------------
n_simulation_PCA <- c(5, 50, 100) #n smaller 5 is not recommended, since this can lead to errors with the eigenvalue plots
T_simulation_PCA <- c(7, 50, 100) #at least 3 arguments should be supplied to ensure smooth running of the plots of the code
number_samples_PCA <- 50 #must be at least 2
start_seed_PCA <- 1
#-----------------------------------------------
...
#-------------------------------------------------------------------------------------------------
    ...
    #--------------set_up---------------------------
    eigenvalues_to_be_plotted <- 5 #must be smaller than smallest value of n_simulation_PCA in line 3
    ns <- c(5, 50, 100) # must be subset of n_simulation_PCA in line 3
    Ts <- c(7, 50, 100) # must be subset of T_simulation_PCA in line 74, at least 3 arguments should be supplied to ensure smooth running of the code
    #-----------------------------------------------
        width <- 0.6
        ...
        t30_med <- ggplot(list_df[[3]], aes(x=n, y=Magnitude, fill=Eigenvalue)) +
          geom_bar(stat="identity",width = width, position=position_dodge())+
          scale_fill_brewer(palette="Paired") + ggtitle('T=100') + theme(text = element_text(size=22))
        #t100 <- ggplot(list_df[[4]], aes(x=n, y=Magnitude, fill=Eigenvalue)) +
        #  geom_bar(stat="identity",width = width, position=position_dodge())+
        #  scale_fill_brewer(palette="Paired") + ggtitle('T=100') + theme(text = element_text(size=20)) + ylab('')
    ...
    #--------------set_up---------------------------
    ns <-  c(5, 50, 100) # must be subset of n_simulation_PCA in line 73
    Ts <-  c(7, 50, 100) # must be subset of T_simulation_PCA in line 74
    #-----------------------------------------------
    ...
    start_seed <- 1
    number_samples_time <- 2
    number_of_signals <- c(5, 10, 20)
    #-----------------------------------------------
...
#rm(list = ls()) #if desired remove all elements from environment
    ...
    #--------------set_up---------------------------
    set.seed(1)
    n <- 100 #should be greater than 20
    T <- 50
    q <- 10
    #-----------------------------------------------
        ...
        df <- as.data.frame(rbind(BIC_n_df_plot[1:(n-10),], BIC_T_df_plot[1:(n-10),], BIC_nT_df_plot[1:(n-10),]))
        df['IC'][df['IC'] == 1] <- 'BIC_n'
        df['IC'][df['IC'] == 2] <- 'BIC_T'
        ...
        graphics.off() #clear all other plots
        (fig3 <- ggplot(BNIC_df_plot[1:(n-10),], aes(x=r, y=mean,  lty = 'BNIC')) + geom_line() +
