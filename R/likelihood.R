...
wrapper <- function(theta,data,n,p,q,t){
  ...
  # save first values
  post_f[[1]] <- post_wrapper_inital(lambda=lambda,sigma_u=sigma_u,f_inital=f_init(q=q,p=p),sig_f_inital=sigma_f_init(p=p,q=q,gamma=gamma,sigma_e=sigma_e),data = data[,1])$f_post
  post_sigma_f[[1]] <- post_wrapper_inital(lambda=lambda,sigma_u=sigma_u,f_inital=f_init(q=q,p=p),sig_f_inital=sigma_f_init(p=p,q=q,gamma=gamma,sigma_e=sigma_e),data = data[,1])$sigma_f_post
  ...
  for(i in 2:t){
    ...
    #save second values
    post_f[[i]] <- post_wrapper(gamma=gamma,lambda=lambda,sigma_u=sigma_u,sigma_e=sigma_e,f_post_last=post_f[[i-1]],sigma_f_post_last=post_sigma_f[[i-1]],  data = data[,i])$f_post
    post_sigma_f[[i]] <- post_wrapper(gamma=gamma,lambda=lambda,sigma_u=sigma_u,sigma_e=sigma_e,f_post_last=post_f[[i-1]],sigma_f_post_last=post_sigma_f[[i-1]],  data = data[,i])$sigma_f_post
  }
...
#theta_wrapper(keks$par,n=n,p=p,q=q)