...
start_ar=4
gamma <- random_matrices(n = 1, row = (p+1)*q, col = (p+1)*q) #n= 1 einfachshalber
cov_stationarity(gamma,epsilon = 0.05)
F <- sim_F(dim = q, lags = p, T = T)$F
Stack<- stack_F(F = F, lags = 3, start_ar=start_ar)
X= sim_X(F = F, p = n, lags = lags_obs_var, start_ar = start_ar)

lambda<- random_matrices(n=1, row = n, col = q*(p+1))
#Sigma_eta=matrix(rep(diag(q),p), nrow = q, ncol = (p+1)*q)
#Sigma_e=list(matrix(data = rbind(Sigma_eta,matrix(data=0,nrow = (p+1)*q-q, ncol = (p+1)*q)), nrow = (p+1)*q, ncol=(p+1)*q))
Sigma_e=list(diag((p+1)*q))
Sigma_u= list(diag(n)) '
...
Kalman= function(q,p,T,n, lambda, gamma, Sigma_e, Sigma_u ,start_ar,X){
  ...
  ## for completeness add Fsmooth/Psmooth[[T-start_ar]] = post_F/P[[3T-start_ar]]. Not from the paper but intuitivly that might be reasonable
  Fsmooth[[T-start_ar]]<- post_F[[T-start_ar]]
  Psmooth[[T-start_ar]]<- post_P[[T-start_ar]]

  #smoother goes from (T-start_ar) until 2
  Smoother <- function(i){
...
# Result=Kalman(q=q, p=p, T=T, n=n, lambda = lambda, gamma = gamma, Sigma_e = Sigma_e, Sigma_u = Sigma_u, start_ar =start_ar, X=X)
